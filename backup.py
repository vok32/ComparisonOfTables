...
def load_sheets(file_path):
    try:
        ...
        messagebox.showerror("Ошибка", f"Не удалось загрузить листы из файла: {file_path}\nОшибка: {str(e)}")
        return []

def get_sheets_from_file(file_path):
    try:
        workbook = load_workbook(file_path, read_only=True)
        return workbook.sheetnames  # Возвращает список названий листов
    except Exception as e:
        print(f"Ошибка при получении листов: {e}")
        return []

def compare_excel_tables(file1_path, file2_path, output_path, save_option, key_column, root, position, sheet1_name, sheet2_name):
...
def select_files(root):
    ...
    def select_file1():
        global file_path_1  # Объявляем переменную глобальной
        ...
        if filename:
            ...
            file_path_1 = filename.replace("\\", "/")  # Заменяем обратные слэши на прямые

            if select_sheet_var.get():  # Обновляем, если чекбокс установлен
                update_sheet_combobox(sheet1_combo, filename)

    def select_file2():
        global file_path_2  # Объявляем переменную глобальной
        filename = filedialog.askopenfilename(filetypes=[("Excel files", "*.xlsx;*.xls")])
        if filename:
            ...
            # Преобразуем путь к файлу с использованием прямых слэшей
            file_path_2 = filename.replace("\\", "/")  # Заменяем обратные слэши на прямые

            if select_sheet_var.get():  # Обновляем, если чекбокс установлен
                update_sheet_combobox(sheet2_combo, filename)
    ...
    def update_sheet_combobox(combobox, file_path):
        try:
            workbook = load_workbook(file_path, read_only=True)
            sheet_names = workbook.sheetnames
            combobox['values'] = sheet_names
            if sheet_names:
                combobox.current(0)  # Устанавливаем первый лист по умолчанию
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось загрузить листы из файла: {file_path}\nОшибка: {str(e)}")

    def toggle_sheet_selection():
        if select_sheet_var.get():
            sheet1_combo.grid(row=2, column=1, padx=5, pady=5)
            sheet2_combo.grid(row=3, column=1, padx=5, pady=5)
        else:
            sheet1_combo.grid_remove()
            sheet2_combo.grid_remove()
    ...
    def show_columns_selection():
        ...
        # Если не выбраны листы, использовать значения по умолчанию
        sheet1_name = sheet1_combo.get() if select_sheet_var.get() else None
        sheet2_name = sheet2_combo.get() if select_sheet_var.get() else None
        ...
        def start_comparison():
            ...
            sheet1_name = sheet1_combo.get() if select_sheet_var.get() else None
            sheet2_name = sheet2_combo.get() if select_sheet_var.get() else None
            compare_excel_tables(file1_path, file2_path, output_path, save_option, key_column, root, position, sheet1_name, sheet2_name)
    ...
    file1_entry = Entry(frame, width=60)
    ...
    file2_entry = Entry(frame, width=60)
    ...
    global sheet1_label, sheet1_combo, sheet2_label, sheet2_combo
    sheet1_label = None
    sheet1_combo = None
    sheet2_label = None
    sheet2_combo = None
    ...
    def toggle_sheet_selection():
        ...
        if select_sheet_var.get():
            if sheet1_label is None or sheet2_label is None:
                sheet1_label = Label(frame, text="Выберите лист первого файла:")
                sheet1_label.grid(row=2, column=0, sticky=W)
                sheet1_combo = ttk.Combobox(frame, width=57)
                sheet1_combo.grid(row=2, column=1, padx=5, pady=5)

                sheet2_label = Label(frame, text="Выберите лист второго файла:")
                sheet2_label.grid(row=3, column=0, sticky=W)
                sheet2_combo = ttk.Combobox(frame, width=57)
                sheet2_combo.grid(row=3, column=1, padx=5, pady=5)

            update_sheet_comboboxes()
        else:
            if sheet1_label is not None:
                sheet1_label.grid_remove()
                sheet1_combo.grid_remove()
                sheet2_label.grid_remove()
                sheet2_combo.grid_remove()

    def update_sheet_comboboxes():
        update_sheet_combobox(sheet1_combo, file_path_1)
        update_sheet_combobox(sheet2_combo, file_path_2)
        sheets_file1 = get_sheets_from_file(file_path_1)
        sheets_file2 = get_sheets_from_file(file_path_2)

        sheet1_combo['values'] = sheets_file1
        sheet2_combo['values'] = sheets_file2
    
    # file_path_1_ = file1_entry.get()
    # file_path_2_ = file2_entry.get()
        
    # Чекбокс для выбора листа
    select_sheet_var = BooleanVar()
    select_sheet_check = Checkbutton(frame, text="Выбрать лист", variable=select_sheet_var, command=toggle_sheet_selection)
    select_sheet_check.grid(row=4, column=0, columnspan=2, sticky=W)

    output_label = Label(frame, text="Выберите папку для сохранения:")
    output_label.grid(row=5, column=0, sticky=W)

    output_entry = Entry(frame, width=60)
    ...
    filename_button.grid(row=6, column=2, padx=5, pady=5)
    ...
    save_radio5.grid(row=11, column=1, columnspan=2, padx=5, pady=5, sticky=W)
    ...
    save_label.grid(row=11, column=2, sticky=E, pady=10)
